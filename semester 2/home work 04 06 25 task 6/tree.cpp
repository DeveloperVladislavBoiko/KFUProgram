...
stack<node*> find_in_tree(node* root, double value);
bool remove_from_tree(node*& root, double value);
...
node* find_in_tree(tree& t, double value)
{
    ...
    return (result->bal == value) ? result : nullptr;
}
...
stack<node*> find_in_tree(node* root, double value) {
    ...
    auto curr = root;
    st.push(root);
    while (curr && value != curr->bal) {
        if (value < curr->bal) curr = curr->left;
...
bool remove_from_tree(node*& root, double value) {
    ...
    if (rem_node->bal != value) return false;
