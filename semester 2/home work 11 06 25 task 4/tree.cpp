...
bool add_to_tree(node*& root, int value, bool balance_tree);
stack<node*> find_in_tree(node* root, int value);
bool remove_from_tree(node*& root, int value, bool balance_tree);
...
bool add_to_tree(tree& t, int value, bool balance_tree)
{
    auto res = add_to_tree(t.root, value, balance_tree);
    if (res) {
        t.count++;
...
bool remove_from_tree(tree& t, int value, bool balance_tree = true)
{
    auto res = remove_from_tree(t.root, value, balance_tree);
    if (res) {
        t.count--;
        t.height = get_level(t.root) + 1;
    }
...
bool add_to_tree(node*& root, int value, bool balance_tree) {
    ...
    if (value < result->value) 
        result->left = create_node(value);
    else 
        result->right = create_node(value);
